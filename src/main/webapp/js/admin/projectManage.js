...
$(document).ready(function () {
    ...
    $('#table').bootstrapTable({
        url: "../admin/projectDocument/select?projectBoxId=" + encodeURIComponent(boxId),
        pagination: true,
    ...
    $('#docSubmit-change').click(function () {
        ...
        if (documentTime !== "" && responsiblePerson !== "" && title !== "" && keepTime !== "" && createUserName !== "" ) {
            $.ajax({
                type: 'post',
                url: "../admin/projectDocument/update",
                data: {
                    "boxNumber": boxNum,
                    "id": $('#docId1').val(),
                    "number": fileNum,
    ...
    $('#search').click(function () {
        ...
        $('#table').bootstrapTable({
            url: "../admin/projectDocument/select?" + str + "&projectBoxId=" + encodeURIComponent(boxId),
            pagination: true,
            pageNumber: 1, //初始化加载第一页，默认第一页
            pageSize: 5, //每页的记录行数（*）
            pageList: [5, 10, 15],//可供选择的每页行数
    ...
    $('#search-whole').click(function () {
        ...
        $('#table').bootstrapTable('destroy')
        $('#table').bootstrapTable({
            url: "../admin/projectDocument/selectProjectData?" + str ,
            pagination: true,
            pageNumber: 1, //初始化加载第一页，默认第一页
            pageSize: 5, //每页的记录行数（*）
            pageList: [5, 10, 15],//可供选择的每页行数
...
function operateFormatter3(value, row, index) {
    return '<img style="max-height:200px;max-width:200px" src="../admin/projectContent/getImage/' + row.id + '"/>'
}
...
window.operateEvents = {
    'click .selectImg': function (e, value, row, index) {
        ...
        $('#table2').bootstrapTable({
            url: "../admin/projectContent/select?projectDocumentId=" + row.id,
            pagination: false,
            pageNumber: 1, //初始化加载第一页，默认第一页
            pageSize: 5, //每页的记录行数（*）
            pageList: [5, 10, 15],//可供选择的每页行数
    ...
    'click .remove': function (e, value, row, index) {
        ...
        if (confirm("您确定要删除此档案吗？\n该档案下的所有图片将被删除！")) {
            $.ajax({
                type: "DELETE",
                url: "../admin/projectDocument/delete/" + row.id,
                success: function (data) {
                    $("#table").bootstrapTable("refresh");
                }
            });
...
window.operateEvents1 = {
    'click .removeImg': function (e, value, row, index) {
        if (confirm("您确定要删除这张图片吗？")) {
            $.ajax({
                type: "DELETE",
                url: "../admin/projectContent/delete/" + row.id,
                success: function (data) {
...
function upload() {
    ...
    Dragfiles.forEach(function (value, key) {  //循环上传
        ...
        $.ajax({
            url: '../admin/projectContent/uploadImage',
            type: 'POST',
            data: form,
            async: false,
            cache: false,
