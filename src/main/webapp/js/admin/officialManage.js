...
$(document).ready(function () {
    ...
    $('#table').bootstrapTable({
        url: "../admin/officialDocument/select?officialBoxId=" + encodeURIComponent(boxId),
        pagination: true,
    ...
    $('#docSubmit').click(function () {
        ...
        if (documentTime !== "" && responsiblePerson !== "" && title !== "" && keepTime !== "" && createUserName !== "" ) {
            $.ajax({
                type: 'post',
                url: "../admin/officialDocument/add",
                data: {
                    "officialBoxId": boxId,
                    "boxNumber": boxNum,
                    "number": fileNum,
    ...
    $('#docSubmit-change').click(function () {
        ...
        if (documentTime !== "" && responsiblePerson !== "" && title !== "" && keepTime !== "" && createUserName !== "" ) {
            $.ajax({
                type: 'post',
                url: "../admin/officialDocument/update",
                data: {
                    "boxNumber": boxNum,
                    "id": $('#docId1').val(),
                    "number": fileNum,
    ...
    $('#search').click(function () {
        ...
        $('#table').bootstrapTable({
            url: "../admin/officialDocument/select?" + str + "&officialBoxId=" + encodeURIComponent(boxId),
            pagination: true,
            pageNumber: 1, //初始化加载第一页，默认第一页
            pageSize: 5, //每页的记录行数（*）
            pageList: [5, 10, 15],//可供选择的每页行数
    ...
    $('#search-whole').click(function () {
        ...
        $('#table').bootstrapTable('destroy')
        $('#table').bootstrapTable({
            url: "../admin/officialDocument/select?" + str ,
            pagination: true,
            pageNumber: 1, //初始化加载第一页，默认第一页
            pageSize: 5, //每页的记录行数（*）
            pageList: [5, 10, 15],//可供选择的每页行数
...
function operateFormatter3(value, row, index) {
    return '<img style="max-height:200px;max-width:200px" src="../admin/officialContent/getImage/' + row.id + '"/>'
}
...
function operateFormatter1(value, row, index){
    return value.toString().substring(41);
}
...
window.operateEvents = {
    'click .selectImg': function (e, value, row, index) {
        ...
        $('#table2').bootstrapTable({
            url: "../admin/officialContent/select?officialDocumentId=" + encodeURIComponent(row.id) + "&officialBoxId=" + encodeURIComponent(boxId),
            pagination: false,
            pageNumber: 1, //初始化加载第一页，默认第一页
            pageSize: 5, //每页的记录行数（*）
            pageList: [5, 10, 15],//可供选择的每页行数
    ...
    'click .remove': function (e, value, row, index) {
        ...
        if (confirm("您确定要删除此档案吗？\n该档案下的所有图片将被删除！")) {
            $.ajax({
                type: "DELETE",
                url: "../admin/officialDocument/delete/" + row.id,
                success: function (data) {
                    $("#table").bootstrapTable("refresh");
                }
            });
...
window.operateEvents1 = {
    'click .removeImg': function (e, value, row, index) {
        if (confirm("您确定要删除这张图片吗？")) {
            $.ajax({
                type: "DELETE",
                url: "../admin/officialContent/delete/" + row.id,
                success: function (data) {
...
dz.ondrop = function (ev) {
    ...
    while (i < len) {
        ...
        tr.innerHTML = '<td><a target="_blank" href="' + window.URL.createObjectURL(files[i]) + '">' + files[i].name + '</a></td><td>' + time + '</td><td>' + size + '</td><td>删除</td>';
        frag.appendChild(tr);
        //添加文件到newForm
...
function upload() {
    ...
    Dragfiles.forEach(
        function (value, key) {  //循环上传
            var form = new FormData();
            form.append("id", $('#docId2').val());
            form.append("image", value)
            $.ajax({
                url: '../admin/officialContent/uploadImage',
                type: 'POST',
                data: form,
                async: false,
                cache: false,
